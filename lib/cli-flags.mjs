 ...
 * @param {{valueFlags?: string[], exitCode?: number}} [options] - `valueFlags`
 *   lists flags whose space-separated value is the NEXT argv token (e.g.
 *   `--since 7`). Without this, a value that itself starts with `-` (e.g.
 *   `--since -5`, a negative day count) would be misread as an unrecognized
 *   flag rather than left for the caller's own value validation to reject
 *   with a clearer message. `--flag=value` is self-contained and never needs
 *   to be listed. `exitCode` (default 1) is the status this call exits with
 *   on an unrecognized flag or a missing operand — a caller whose own usage-
 *   error convention differs (analyze-patterns.mjs uses 2, #3113) passes it
 *   explicitly rather than forking the detection logic to get a different exit code.
 ...
 */
export function validateFlags(args, knownFlags, usage, { valueFlags = [], requireOperand = false, exitCode = 1 } = {}) {
